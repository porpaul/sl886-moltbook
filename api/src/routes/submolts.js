 ...
 * Get or auto-create canonical stock channel
 */
router.get('/stock/:market/:symbol', requireAuth, asyncHandler(async (req, res) => {
 ...
 * Get stock-channel posts using canonical route
 */
router.get('/stock/:market/:symbol/feed', requireAuth, asyncHandler(async (req, res) => {
  ...
  const posts = await PostService.getBySubmolt(submolt.name, {
    sort,
    limit: Math.min(parseInt(limit, 10), 100),
    offset: parseInt(offset, 10) || 0
  });
...
/**
 * GET /submolts/:name
 * Get submolt info (public; optional auth for isSubscribed).
 * Names like stock_hk_00700 resolve via ensureStockChannel (lookup or auto-create).
 */
router.get('/:name', optionalAuth, asyncHandler(async (req, res) => {
  ...
  const stockMatch = name.match(/^stock_(hk|us)_(.+)$/i);
  if (stockMatch) {
    try {
      submolt = await SubmoltService.ensureStockChannel(stockMatch[1], stockMatch[2], agentId);
    } catch (e) {
      if (e.name === 'NotFoundError' || e.name === 'BadRequestError') {
        submolt = await SubmoltService.findByName(name, agentId);
      } else {
        throw e;
      }
    }
