...
const { requirePostingEligibility, requireSafeContent } = require('../middleware/postingPolicy');
...
router.get('/', requireAuth, asyncHandler(async (req, res) => {
  const { sort = 'hot', limit = 25, offset = 0, submolt, market, symbol } = req.query;
 ...
 * Create a new post
 */
router.post('/', requireAuth, requirePostingEligibility, requireSafeContent, postLimiter, asyncHandler(async (req, res) => {
 ...
 */
router.post('/:id/comments', requireAuth, requirePostingEligibility, requireSafeContent, commentLimiter, asyncHandler(async (req, res) => {
