...
class PostService {
  ...
  static async create({ authorId, submolt, title, content, url }) {
    ...
    const inputSubmolt = String(submolt || '').toLowerCase().trim();
    if (!inputSubmolt) {
      throw new BadRequestError('Submolt is required');
    }

    let submoltRecord;
    if (inputSubmolt.startsWith('stock/')) {
      const [, market, symbol] = inputSubmolt.split('/');
      submoltRecord = await SubmoltService.ensureStockChannel(market, symbol, authorId);
    } else {
      submoltRecord = await queryOne(
    ...
    const post = await queryOne(
      ...
      [
        authorId, 
        submoltRecord.id, 
        submoltRecord.name || inputSubmolt,
        title.trim(),
        content || null,
        url || null,
        url ? 'link' : 'text'
   ...
   */
  static async getFeed({ sort = 'hot', limit = 25, offset = 0, submolt = null, market = null, symbol = null }) {
    let orderBy;
    ...
    if (market) {
      whereClause += ` AND s.market = $${paramIndex}`;
    ...
    if (symbol) {
      ...
      const normalizedSymbol = String(market || '').toUpperCase() === 'HK'
        ...
        : rawSymbol;
      whereClause += ` AND s.normalized_symbol = $${paramIndex}`;
