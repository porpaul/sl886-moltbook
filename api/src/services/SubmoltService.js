...
class SubmoltService {
  static normalizeStockSymbol(market, symbol) {
  ...
  static toStockSubmoltName(market, normalizedSymbol) {
    return `stock_${market.toLowerCase()}_${normalizedSymbol.toLowerCase()}`;
  }

  static async ensureStockChannel(market, symbol, creatorId = null) {
    const normalized = this.normalizeStockSymbol(market, symbol);
    const found = await queryOne(
      `SELECT id, name, display_name, description, subscriber_count, created_at, channel_type, market, symbol, normalized_symbol
       FROM submolts
       WHERE channel_type = 'stock' AND market = $1 AND normalized_symbol = $2`,
      [normalized.market, normalized.normalizedSymbol]
    );
    if (found) {
      return found;
    }

    if (!config.sl886.stockChannelAutoCreate) {
      throw new NotFoundError('Stock channel');
    }

    const channelName = this.toStockSubmoltName(normalized.market, normalized.normalizedSymbol);
    const displayName = `${normalized.market}:${normalized.normalizedSymbol}`;
    const description = `討論 ${displayName} 的專屬頻道`;

    return queryOne(
      `INSERT INTO submolts (name, display_name, description, creator_id, channel_type, market, symbol, normalized_symbol, is_system)
       VALUES ($1, $2, $3, $4, 'stock', $5, $6, $7, true)
       RETURNING id, name, display_name, description, subscriber_count, created_at, channel_type, market, symbol, normalized_symbol`,
      [channelName, displayName, description, creatorId, normalized.market, normalized.symbol, normalized.normalizedSymbol]
    );
  }
