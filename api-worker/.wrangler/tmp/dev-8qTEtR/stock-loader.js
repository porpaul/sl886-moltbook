...
async function ensureStockChannel(env2, market, symbol, creatorId = null, displayName) {
  ...
  const displayNameStr = displayName != null && String(displayName).trim() ? `${String(displayName).trim()} (${codeLabel})` : codeLabel;
...
// src/stock-loader.ts
var WORKER_NAME = "sl886-moltbook-stock-loader";
async function loadStocks(env2) {
  const url = env2.SL886_STOCKS_URL?.trim();
  if (!url) {
  ...
  if (env2.STOCK_LOADER_SECRET) {
    headers["X-Stock-Loader-Secret"] = env2.STOCK_LOADER_SECRET;
  }
  ...
  const result = { fetched: items.length, inserted: 0, skipped: 0, errors: 0 };
  ...
  result.deletedWrongHK = await deleteWrongHKStockChannels(env2);
  return result;
}
__name(loadStocks, "loadStocks");
async function deleteWrongHKStockChannels(env2) {
  const usSymbols = /* @__PURE__ */ new Set();
  ...
  for (const r of hkRows) {
    const n = String(r.normalized_symbol ?? "").trim();
...
__name(deleteWrongHKStockChannels, "deleteWrongHKStockChannels");
var stock_loader_default = {
  async scheduled(_event, env2, _ctx) {
    try {
      const result = await loadStocks(env2);
      console.log(
  ...
  async fetch(request, env2, ctx) {
    ...
    if (url.pathname === "/trigger") {
      ...
      if (env2.STOCK_LOADER_SECRET && secret !== env2.STOCK_LOADER_SECRET) {
        return new Response(
...
// .wrangler/tmp/bundle-cPqTRr/middleware-insertion-facade.js
var __INTERNAL_WRANGLER_MIDDLEWARE__ = [
  middleware_ensure_req_body_drained_default,
  middleware_miniflare3_json_error_default
];
var middleware_insertion_facade_default = stock_loader_default;
...
// .wrangler/tmp/bundle-cPqTRr/middleware-loader.entry.ts
...
//# sourceMappingURL=stock-loader.js.map
